...
class Doctor_Service_ApiGenerator_ApiGenerator 
implements 
    Doctor_Service_Interface_ApiGenerator 
{
    ...
    public function __construct($siteRootOffset = '') {
        ...
        if (!is_writable($this->apiDirectory)) {
            throw new Doctor_Service_ApiGenerator_Exception('У PHP нет прав на запись в директорию '.$this->apiDirectory);
        }
    ...
    public function generate() {
        ...
        foreach ($classes as $namespace => $classNames) {
            ...
            if (!in_array('main', $classNames)) {
                throw new Doctor_Service_ApiGenerator_Exception('В директории для '.$namespace.' не хватает файла main.php');
            }
            ...
            foreach ($classNames as $className) {
                ...
                if (!class_exists($class, true)) {
                    throw new Doctor_Service_ApiGenerator_Exception('Класс для обработки RPC-вызовов не обнаружен: ' . $class);
                }
        ...
        if(!is_writable($this->javascriptApiFile)) {
            throw new Doctor_Service_ApiGenerator_Exception('Не возможно изменить файл '.$this->javascriptApiFile);
        }
        
        if(!is_writable($this->phpApiFile)) {
            throw new Doctor_Service_ApiGenerator_Exception('Не возможно изменить файл '.$this->phpApiFile);
        }
